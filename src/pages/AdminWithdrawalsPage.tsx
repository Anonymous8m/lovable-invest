...
const AdminWithdrawalsPage = () => {
  ...
  const handleAction = async (withdrawal: PendingWithdrawal, action: "completed" | "rejected") => {
    ...
    if (action === "completed") {
      // Deduct balance and credit total_withdrawal on approval
      ...
      if (profile) {
        await supabase
          ...
          .update({
            ...
            total_withdrawal: (profile.total_withdrawal ?? 0) + withdrawal.amount,
          })
          .eq("id", withdrawal.user_id);
      }
    }
    ...
    toast({
      ...
      description: `$${withdrawal.amount.toLocaleString()} withdrawal has been ${action}.`,
    });
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-display font-bold text-foreground flex items-center gap-2">
            <ShieldCheck className="w-6 h-6 text-primary" /> Withdrawal Approvals
          </h1>
          <p className="text-muted-foreground text-sm mt-1">Review and approve pending withdrawal requests</p>
        </div>
